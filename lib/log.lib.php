...
function log_event($username, $event_name, $event_description, $log_name)
{
    ...
    $log_filename = str_replace("%Y", date('Y'), (int) $log_filename);
    $log_filename = str_replace("%m", date('m'), (int) $log_filename);
    $log_filename = str_replace("%d", date('d'), (int) $log_filename);
